...
namespace framework
{
	...
	void c_multidropdown::draw()
	{
		...
		g_render->use_layer(m_layer, [&]()
			{
				...
				g_render->rect_filled((m_pos + position).x, (m_pos + position).y, m_child_size, 25.f, g_style->m_element_base.modulate(animations::m_window_opacity.val()), 2.f);
				g_render->rect((m_pos + position).x, (m_pos + position).y, m_child_size, 25.f, g_style->m_outline.modulate(animations::m_window_opacity.val()), 2.f);
				...
				if (preview_selection.empty()) {
					...
					const unsigned int wrapped_length = g_font->f_childs.get_font()->CalcWordWrapPositionA(1.f, &preview_selection[0], preview_selection.data() + preview_selection.length(),
				...
				g_font->f_childs.text((m_pos + position + math::c_vector_2d(6, 5.5)).x, (m_pos + position + math::c_vector_2d(5, 3)).y, preview_selection, g_style->m_text.modulate(animations::m_window_opacity.limit(0.3f).val()));
		...
		{
			auto position2 = m_hide_label ? math::c_vector_2d(0, 0) : math::c_vector_2d(0, g_font->f_childs.measure(m_label).y + 33);
			...
			g_render->use_layer(engine::render_layer::prioritized, [&]()
				{
					g_render->rect_filled((m_pos + position2).x, (m_pos + position2).y, calculated_size.x, calculated_size.y, g_style->m_element_base.modulate(animations::m_dropdown_open.val()), 2.f);
					g_render->rect((m_pos + position2).x, (m_pos + position2).y, calculated_size.x, calculated_size.y, g_style->m_outline.modulate(animations::m_dropdown_open.val()), 2.f);
					...
					for (int i = 0; i < this->m_data.size(); i++)
					{
						...
						g_font->f_childs.text(m_pos.x + position2.x + 5, (float)y, dropdown.m_label,
