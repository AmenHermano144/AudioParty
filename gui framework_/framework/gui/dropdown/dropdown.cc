...
namespace framework
{
	...
	void c_dropdown::draw()
	{
		...
		if (animations::m_dropdown_open.val() > 0.f)
		{
			...
			auto calculated_size = math::c_vector_2d(m_child_size, (this->m_items.size() * g_font->f_childs.measure(m_label).y + 1) + 6 /* 3 top / 3 bottom */);
			...
			g_render->use_layer(engine::render_layer::prioritized, [&]()
				{
					...
					for (int i = 0; i < this->m_items.size(); i++)
					{
						...
						g_font->f_childs.text(m_pos.x + position2.x + 5, (float)y, dropdown,
