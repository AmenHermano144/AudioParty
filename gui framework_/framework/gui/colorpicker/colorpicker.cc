	...
	void c_colorpicker::draw()
	{
		...
		if (animations::m_colorpicker_value.val() > 0.f)
		{
			...
			g_render->use_layer(engine::render_layer::prioritized, [&]()
			{
				...
				g_render->rect_shadow(pallete_pos.x, pallete_pos.y, pallete_size.x, pallete_size.y, g_style->m_window_shadow.modulate(animations::m_colorpicker_value.val()), 4.f);
				g_render->rect_filled(pallete_pos.x, pallete_pos.y, pallete_size.x, pallete_size.y, g_style->m_window_bars.modulate(animations::m_colorpicker_value.val()), 4.f);
				g_render->rect_filled(pallete_pos.x, pallete_pos.y, pallete_size.x, 25.f, g_style->m_child_top.modulate(animations::m_colorpicker_value.val()), 4.f, engine::draw_flags_::draw_flags_round_corners_top);
