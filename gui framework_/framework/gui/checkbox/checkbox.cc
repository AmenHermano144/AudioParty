...
namespace framework
{
	...
	void c_checkbox::draw()
	{
		...
		g_render->use_layer(m_layer, [&]()
			{
				g_render->rect_filled(m_pos.x, m_pos.y, m_size.y, m_size.y, g_style->m_element_base.modulate(animations::m_window_opacity.val()), 2.f);
				g_render->rect(m_pos.x, m_pos.y, m_size.y, m_size.y, g_style->m_outline.modulate(animations::m_window_opacity.val()), 2.f);

				//g_render->rect_shadow(m_pos.x, m_pos.y, m_size.y, m_size.y, g_style->m_accent.modulate(animations::m_checkbox_value.val()), 10.f, 2.f);
				g_render->rect_filled(m_pos.x + 1, m_pos.y + 1, m_size.y - 2, m_size.y - 2, g_style->m_accent.modulate(animations::m_checkbox_value.val()), 2.f);
				g_render->fade_rect_filled(m_pos.x + 1, m_pos.y + 1, m_size.y - 2, m_size.y - 2, hue::c_color(0, 0, 0, 0), hue::c_color(0, 0, 0, 50 * animations::m_checkbox_value.val()), engine::fade_direction::vertically, 2.f);
				g_render->check_mark(m_pos.x + 5, m_pos.y + 5, 7.f, hue::c_color(0, 0, 0, 255 * animations::m_checkbox_value.val()));

				g_font->f_childs.text(m_pos.x + 25, m_pos.y - 0.5, m_label, g_style->m_text.modulate(final_opacity));
			});
	}
