...
namespace framework
{
	...
	void c_popup::draw()
	{
		...
		if (animations::m_popup_value.val() > 0.f)
		{
			...
			math::c_vector_2d pallete_size = { 180, this->m_height };
			...
			g_render->use_layer(engine::render_layer::middle, [&]()
			{
				...
				g_render->rect_shadow(pallete_pos.x, pallete_pos.y, pallete_size.x, pallete_size.y, g_style->m_window_shadow.modulate(animations::m_popup_value.val()), 4.f);
				g_render->rect_filled(pallete_pos.x, pallete_pos.y, pallete_size.x, pallete_size.y, g_style->m_window_bars.modulate(animations::m_popup_value.val()), 4.f);
				g_render->rect_filled(pallete_pos.x, pallete_pos.y, pallete_size.x, 25.f, g_style->m_child_top.modulate(animations::m_popup_value.val()), 4.f, engine::draw_flags_::draw_flags_round_corners_top);
				...
				for (auto control : this->m_controls)
				{
					...
					if (control->m_inlined)
					{
						...
						{
							// colorpickers can only be inlined to checkboxes, keybinds, other colopickers, if its anything else just disable inlining
							//if (m_parent_control->m_type != element_type::checkbox || m_parent_control->m_type != element_type::colorpicker || m_parent_control->m_type != element_type::keybind)
							//{
