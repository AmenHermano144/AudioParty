#include "gui framework_/includes.hh"
#include "audio/AudioRouter.h"

#include "imgui_impl_win32.h"
#include "imgui_impl_dx11.h"

#include <d3d11.h>
#include <dwmapi.h>
#pragma comment(lib, "d3d11.lib")
#pragma comment(lib, "dwmapi.lib")

// stubs for custom imgui_widgets.cpp externs (dx9 project leftovers)
float accent_color[3] = { 1.0f, 1.0f, 1.0f };
ImFont* ico = nullptr;
ImFont* ico_button = nullptr;
ImFont* ico_combo = nullptr;
ImFont* default_segu = nullptr;
int tab_count = 0;

// dx11 globals
static ID3D11Device* g_device = nullptr;
static ID3D11DeviceContext* g_device_context = nullptr;
static IDXGISwapChain* g_swap_chain = nullptr;
static ID3D11RenderTargetView* g_render_target = nullptr;

// audio router
static std::unique_ptr<AudioRouter> g_router;

// menu dimensions (must match the c_window size in menu.cc)
static constexpr int MENU_WIDTH = 680;
static constexpr int MENU_HEIGHT = 520;

// forward declarations
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
static bool create_device(HWND hwnd);
static void cleanup_device();
static void create_render_target();
static void cleanup_render_target();
static LRESULT WINAPI wnd_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);

AudioRouter* getRouter() { return g_router.get(); }

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
    auto hr = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
    if (FAILED(hr))
        return 1;

    g_router = std::make_unique<AudioRouter>();
    if (!g_router->init()) {
        MessageBoxW(nullptr, L"Failed to initialize audio router.\nMake sure you have at least one audio output device.",
                     L"AudioParty", MB_ICONERROR);
        CoUninitialize();
        return 1;
    }

    WNDCLASSEXW wc{};
    wc.cbSize = sizeof(wc);
    wc.style = CS_CLASSDC;
    wc.lpfnWndProc = wnd_proc;
    wc.hInstance = hInstance;
    wc.lpszClassName = L"AudioPartyHost";
    RegisterClassExW(&wc);

    // borderless popup window sized exactly to the menu
    auto hwnd = CreateWindowExW(
        WS_EX_APPWINDOW,
        wc.lpszClassName, L"AudioParty",
        WS_POPUP | WS_VISIBLE,
        CW_USEDEFAULT, CW_USEDEFAULT, MENU_WIDTH, MENU_HEIGHT,
        nullptr, nullptr, hInstance, nullptr
    );

    // enable dark mode title bar hint for taskbar
    BOOL dark = TRUE;
    DwmSetWindowAttribute(hwnd, 20 /*DWMWA_USE_IMMERSIVE_DARK_MODE*/, &dark, sizeof(dark));

    if (!create_device(hwnd)) {
        cleanup_device();
        UnregisterClassW(wc.lpszClassName, hInstance);
        CoUninitialize();
        return 1;
    }

    ShowWindow(hwnd, SW_SHOWDEFAULT);
    UpdateWindow(hwnd);

    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    auto& io = ImGui::GetIO();
    io.ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard;

    ImGui_ImplWin32_Init(hwnd);
    ImGui_ImplDX11_Init(g_device, g_device_context);

    g_render->set_ctx(g_device_context);
    g_render->setup();

    ImGui_ImplDX11_InvalidateDeviceObjects();

    // pass hwnd to framework so it can move the real window on drag
    framework::g_ctx->m_hwnd = hwnd;
    framework::g_ctx->m_open = true;
    framework::g_menu->initialize();

    slog::log::success("AudioParty ready");

    bool running = true;
    while (running)
    {
        MSG msg;
        while (PeekMessage(&msg, nullptr, 0U, 0U, PM_REMOVE)) {
            TranslateMessage(&msg);
            DispatchMessage(&msg);
            if (msg.message == WM_QUIT)
                running = false;
        }
        if (!running)
            break;

        ImGui_ImplDX11_NewFrame();
        ImGui_ImplWin32_NewFrame();
        ImGui::NewFrame();

        g_render->set_draw_list(ImGui::GetBackgroundDrawList());
        g_render->begin_layers();

        framework::g_menu->runtime();

        g_render->end_layers();

        ImGui::Render();

        constexpr float clear_color[4] = { 0.06f, 0.06f, 0.06f, 1.0f };
        g_device_context->OMSetRenderTargets(1, &g_render_target, nullptr);
        g_device_context->ClearRenderTargetView(g_render_target, clear_color);

        ImGui_ImplDX11_RenderDrawData(ImGui::GetDrawData());

        g_swap_chain->Present(1, 0);
    }

    g_router->shutdown();
    g_router.reset();

    ImGui_ImplDX11_Shutdown();
    ImGui_ImplWin32_Shutdown();
    ImGui::DestroyContext();

    cleanup_render_target();
    cleanup_device();
    DestroyWindow(hwnd);
    UnregisterClassW(wc.lpszClassName, hInstance);

    CoUninitialize();
    return 0;
}

static bool create_device(HWND hwnd)
{
    DXGI_SWAP_CHAIN_DESC sd{};
    sd.BufferCount = 2;
    sd.BufferDesc.Width = MENU_WIDTH;
    sd.BufferDesc.Height = MENU_HEIGHT;
    sd.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    sd.BufferDesc.RefreshRate.Numerator = 60;
    sd.BufferDesc.RefreshRate.Denominator = 1;
    sd.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH;
    sd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
    sd.OutputWindow = hwnd;
    sd.SampleDesc.Count = 1;
    sd.SampleDesc.Quality = 0;
    sd.Windowed = TRUE;
    sd.SwapEffect = DXGI_SWAP_EFFECT_DISCARD;

    UINT flags = 0;
#ifdef _DEBUG
    flags |= D3D11_CREATE_DEVICE_DEBUG;
#endif

    D3D_FEATURE_LEVEL feature_level;
    constexpr D3D_FEATURE_LEVEL levels[] = { D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_0 };

    auto result = D3D11CreateDeviceAndSwapChain(
        nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, flags,
        levels, 2, D3D11_SDK_VERSION,
        &sd, &g_swap_chain, &g_device, &feature_level, &g_device_context
    );

    if (FAILED(result))
        return false;

    create_render_target();
    return true;
}

static void cleanup_device()
{
    if (g_swap_chain) { g_swap_chain->Release(); g_swap_chain = nullptr; }
    if (g_device_context) { g_device_context->Release(); g_device_context = nullptr; }
    if (g_device) { g_device->Release(); g_device = nullptr; }
}

static void create_render_target()
{
    ID3D11Texture2D* back_buffer = nullptr;
    g_swap_chain->GetBuffer(0, IID_PPV_ARGS(&back_buffer));
    g_device->CreateRenderTargetView(back_buffer, nullptr, &g_render_target);
    back_buffer->Release();
}

static void cleanup_render_target()
{
    if (g_render_target) { g_render_target->Release(); g_render_target = nullptr; }
}

static LRESULT WINAPI wnd_proc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    if (ImGui_ImplWin32_WndProcHandler(hWnd, msg, wParam, lParam))
        return true;

    switch (msg) {
    case WM_SIZE:
        if (g_device && wParam != SIZE_MINIMIZED) {
            cleanup_render_target();
            g_swap_chain->ResizeBuffers(0, LOWORD(lParam), HIWORD(lParam), DXGI_FORMAT_UNKNOWN, 0);
            create_render_target();
        }
        return 0;
    case WM_DESTROY:
        PostQuitMessage(0);
        return 0;
    }

    return DefWindowProcW(hWnd, msg, wParam, lParam);
}
